...
interface DoiNguBacSiErrorProps {
  error?: Error | null;
  onRetry?: () => void;
  isRetrying?: boolean;
}

export function DoiNguBacSiError({
  error,
  onRetry,
  isRetrying = false,
}: DoiNguBacSiErrorProps) {
  const getErrorInfo = () => {
    if (!error) {
      return {
        title: "Đã xảy ra lỗi",
        description:
          "Không thể tải danh sách đội ngũ bác sĩ. Vui lòng thử lại.",
        icon: AlertTriangle,
        color: "text-red-500",
      };
    }
    ...
    if (message.includes("not found") || message.includes("404")) {
      return {
        ...
        description:
          "Danh sách đội ngũ bác sĩ hiện không khả dụng. Vui lòng thử lại sau.",
        icon: Search,
        color: "text-blue-500",
      };
    }
